...
k8s-1에서는 대역만 `192.168.121.x`로 바꾼다 (Stage 9).
...
> 이 **"인터페이스별 NAT 구분"** 이 Stage 9의 핵심이다.
> 인터넷은 NAT, 상대 VM은 NAT 없이. 지금 이해해두면 Stage 9가 수월하다.
...
> 💡 **VM 안의 인터페이스 이름은 호스트와 다르다.**
> 호스트가 `ens3`여도 VM은 `enp1s0`로 잡힌다 (가상 하드웨어 구성이 달라서).
> `--node-ip`나 CNI 인터페이스를 지정할 때 VM 안에서 직접 확인할 것.
...
> ✅ **백킹 파일 + 내부 스냅샷은 정상 동작한다** (2026-09-10, Ubuntu 26.04 / libvirt 12.0.0 실측).
> 일부 환경에서 거부된다고 알려져 있으나 이 구성에서는 문제없었다.
> 실패한다면 백킹 없이 전체 복사본으로 만들거나 외부 스냅샷(`--disk-only`)을 쓴다.
