...
Stage 11의 고장/복구 훈련(CKA 배점 30%)이 전부 여기 기대고 있다.
...
Stage 9에서 k8s-1의 VM이 k8s-2의 VM에 접속하면:
...
**문 ② `wg0` — 상대 호스트의 VM으로** (Stage 9) → 주소를 **바꾸면 안 된다**
...
**되돌리기가 되는지 반드시 확인하고 넘어간다.** 이게 안 되면 Stage 11이 성립하지 않는다.
...
`netfilter-persistent save`를 빠뜨린 것이다. **여기서 잡아야 Stage 9에서 안 헤맨다.**
...
Stage 3·9에서 이 절차를 여러 번 반복하게 된다.
...
| `k1-cp1` | `52:54:00:00:01:11` | 192.168.121.11 | control plane (Stage 9) |
| `k1-cp2` | `52:54:00:00:01:12` | 192.168.121.12 | control plane (Stage 9) |
| `k1-w1` | `52:54:00:00:01:21` | 192.168.121.21 | worker (Stage 9) |
