...
`--control-plane-endpoint` 때문이고, Stage 10에서 CP를 늘려도 설정을 바꾸지 않기 위해서다.
...
control plane이 Stage 10에서 3대가 되므로 `--control-plane-endpoint`가 필요하다.
이 주소는 **클러스터의 영구 주소**가 되어 인증서 SAN과 모든 kubeconfig에 박힌다.

**나중에 바꾸려면 인증서를 전부 재발급해야 한다.**
지금 HAProxy를 거치게 잡아두면 Stage 10에서 클러스터를 다시 만들지 않아도 된다.
...
Stage 10에서 CP를 3대로 늘려도 **각 노드는 설정을 하나도 바꾸지 않는다.**
...
| `--upload-certs` | 인증서를 Secret으로 올려 **Stage 10에서 CP 추가**를 쉽게 한다 |
...
| 항목 | 값 | 이유 |
|---|---|---|
| `mtu` | **1370** | Stage 9의 이중 캡슐화 대비. `wg0` 1420 − VXLAN 50 |
| `interface` | **`enp1s0`** | VM의 인터페이스 이름. 호스트의 `ens3`가 아니다 |
| `cidr` | `10.244.0.0/16` | `--pod-network-cidr`과 일치 |
| `encapsulation` | **VXLAN** | Stage 9에서 WireGuard `AllowedIPs` 필터를 통과하려면 필수 |

> **지금은 단일 노드라 MTU 1370이 필요 없다.** 그런데도 미리 잡는 이유는,
> Stage 9에서 값을 바꾸면 모든 파드를 재시작해야 하고
